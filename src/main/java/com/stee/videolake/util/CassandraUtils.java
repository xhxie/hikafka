package com.stee.videolake.util;
...
public class CassandraUtils {
	 ...
	 */
	public CqlSession getSession() throws IOException,CertificateException, KeyStoreException, NoSuchAlgorithmException, UnrecoverableKeyException, KeyManagementException{
		...
		//use keystore in classpath
		InputStream is = this.getClass().getClassLoader().getResourceAsStream(ssl_keystore_file_path);
		...
		this.session = CqlSession.builder().withSslContext(sc).addTypeCodecs(TypeCodecs.ZONED_TIMESTAMP_UTC)
	...
	private void loadCassandraConnectionDetails() throws Exception {
		...
		ssl_keystore_file_path = config.getProperty("ssl_keystore_file_path");
		String ssl_keystore_password = config.getProperty("ssl_keystore_password");

		// If ssl_keystore_file_path, build the path using JAVA_HOME directory. if
		 ...
		 * 
		 * if (!sslKeyStoreFile.exists() || !sslKeyStoreFile.canRead()) { throw new
		 * Exception( String.format("Unable to access the SSL Key Store file from %s",
